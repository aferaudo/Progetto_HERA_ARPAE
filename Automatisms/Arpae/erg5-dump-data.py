...
Edited: Angelo Feraudo <angelo.feraudo2@unibo.it>
gribapi lib has been deprecated
"""
...
# ERG5 V 1.5 LAST WEEK
# ERG5_FOLDER_ID = "0B7KLnPu6vjdPVGJKR3E4SEluU0U"

# ERG5 V 1.5 SINCE 2001
ERG5_FOLDER_ID = "1NZCBaKL10mrAm4YSPgezhKzChY9d6qXR"

# ERG5 V 1.0 SINCE 2001
# ERG5_FOLDER_ID = "1QZgpY70Ld9HMEzI2i2tDhDe8ORBfKNsK"
...
def get_credentials(clientsecret_path, credential_path, args):
    ...
    if not credentials or credentials.invalid:
        flow = client.flow_from_clientsecrets(clientsecret_path,
                                                           SCOPES)
...
def write_erg5_file(fp, httpclient, refdate):
    ...
    service = discovery.build('drive', 'v3', http=httpclient)
...
def get_product_name(gid):
    ...
    products = {
        ...
        # Precipitazione cumulata oraria osservata al suolo in Kg/m^2
        "prep_hourly_cum": {
            "discipline": 0,
            "parameterCategory": 1,
            "parameterNumber": 52,
        ...
        # Precipitazione cumulata giornaliera osservata al suolo in Kg/m^2
         "prep_daily_cum": {
            "discipline": 0,
            "parameterCategory": 1,
            "parameterNumber": 52,
    ...
    grib_match_key_long = lambda gid, k, v: eccodes.codes_get_long(gid, k) == v if eccodes.codes_is_defined(gid, k) else False
